...
def test_start_script_launches_backend_without_incompatible_calibration_fallback():
    script = (ROOT / "start.ps1").read_text(encoding="utf-8")

    assert "backend.server:app" in script
    assert "api_server:app" not in script
